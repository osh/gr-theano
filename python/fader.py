...
class fader(gr.sync_block):
    ...
    def set_f(self, f):
        ...
        tones = map(lambda x: random.uniform(1,self.fs/4), range(0,self.NS));
        stepval = map(lambda x: numpy.pi*2.0*x/self.fs, tones);
        iv = numpy.vstack( map(lambda x: numpy.exp(1j*numpy.arange(0,self.N*x,x,dtype=numpy.float32), dtype=numpy.complex64), stepval));
        self.set_step( iv );
    ...
    def work(self, input_items, output_items):
        out = output_items[0]
        o = self.rval(len(output_items[0]))
        out[:] = o[0];
